...
if (isDev()) {
    // TODO: move into development env only
    const fs = require('node:fs');
    fs.watch('./feature.html', () => {
        if (featureWindow) featureWindow.reload();
    });
}
...
const windowSize = () => {
    if (isDev()) {
        return {width: 820, height: 448};
    }
    return {width: 420, height: 48};
};

let featureWindow;
const createFeatureWindow = () => {
    featureWindow = new BrowserWindow({
        width: windowSize().width,
        height: windowSize().height,
        backgroundColor: '#d99ea0',
        show: false,
        frame: false,
        resizable: isDev(),
        webPreferences: {
    ...
    featureWindow.on('blur', () => featureWindow.hide());

    if (isDev()) {
        featureWindow.webContents.openDevTools({mode: 'bottom'});
        featureWindow.webContents.send('app:debug', 'focus');
    }
};
...
const loadBookmarks = () => {
    fs.readdir('/Applications', (err, files) => {
        files.forEach(file => {
            file.endsWith('.app') && featureWindow.webContents.send('bookmark:found', file.replace('.app', ''));
        });
    });
}
...
app.on('ready', () => {
    ...
    globalShortcut.register('Cmd+Esc', () => featureWindow.isVisible() ? featureWindow.hide() : showWindow());
    ...
    const contextMenu = Menu.buildFromTemplate([
        {
            ...
            click() {
                featureWindow.isVisible() ? featureWindow.hide() : showWindow();
            },
            accelerator: 'Cmd+Esc'
        },
        {
...
ipcMain.on('bookmark:reload', loadBookmarks);
